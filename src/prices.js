...
export { fetchPriceSnapshot as fetchLivePrices } from './api';
...
export const buildMarketHashName = (item, wear, isStatTrak = false, isSouvenir = false) => {
  ...
  // BUG DÜZELTMESİ: Steam bıçak adında yıldız, StatTrak önekinden ÖNCE gelir.
  const name = item.name.startsWith('★ ') ? `★ ${prefix}${item.name.slice(2)}` : `${prefix}${item.name}`;
...
const lookupLivePrice = (priceMap, item, wear, isStatTrak, isSouvenir) => {
  ...
  return (
    tryKey(!wear || (!item.id?.startsWith('skin-') && !item.weapon) ? item.market_hash_name : null) ??
...
export const getRealisticPrice = (priceMap, item, floatVal, isStatTrak, rarityNameFallback, isSouvenir = false, { stable = false } = {}) => {
  ...
  if (priceMap) {
    ...
    // BUG DÜZELTMESİ: Eksik varyant fiyatı normal skin fiyatıyla doldurulmaz.
...
export const getItemPriceRange = (priceMap, item, rarityNameFallback) => {
  ...
  const low = getRealisticPrice(priceMap, item, maxF, false, rarityNameFallback, false, { stable: true });
  const high = getRealisticPrice(priceMap, item, minF, true, rarityNameFallback, false, { stable: true });
...
const getExpectedPriceForItem = (item, priceMap, rarityOverride, opts = {}) => {
  ...
  for (const tier of WEAR_TIERS) {
    ...
    const base = getRealisticPrice(priceMap, item, mid, false, r, isSouvenir, { stable: true });
    const st = stChance > 0 ? getRealisticPrice(priceMap, item, mid, true, r, isSouvenir, { stable: true }) : base;
