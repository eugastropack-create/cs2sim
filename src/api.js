import { normalizePriceSnapshot } from './priceSnapshot.mjs';
// ============================================================
...
// Fiyat istekleri sınırlı sürede biter; bozuk JSON da yedek kaynağı tetikler.
export async function fetchPriceSnapshot() {
  const sources = [
    ['https://raw.githubusercontent.com/eugastropack-create/skinsimulator/prices-data/latest.json', false],
    ['https://raw.githubusercontent.com/ByMykel/counter-strike-price-tracker/main/static/latest.json', true],
  ];
  let legacy = null;
  sources.splice(1, 0, ['/prices/bootstrap.json', false]);
  for (const [url, fallback] of sources) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 15000);
    try {
      const response = await fetch(url, { signal: controller.signal });
      if (!response.ok) throw new Error('Price HTTP ' + response.status);
      const snapshot = normalizePriceSnapshot(await response.json(), fallback);
      if (!fallback && !Object.keys(snapshot.__snapshot.details).length) { legacy = snapshot; continue; }
      return snapshot;
    } catch (error) { console.warn('Fiyat kaynağı alınamadı:', error.message); }
