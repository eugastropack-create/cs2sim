  ...
  en: {
    ...
    tradeup: {
      ...
      blocks: [
        ...
        { type: 'p', text:
          "Each input float is first normalized within that skin’s own minimum and maximum. The average normalized value is then mapped into the output skin’s range. For example, an input at 0.40 in a 0.10–0.70 range has position 0.50. An output limited to 0.00–0.80 is then 0.40. Calculations retain precision until display; prices follow wear bands rather than individual float premiums." },
        ...
        { type: 'h3', text: 'Five-item Covert contracts' },
        { type: 'p', text:
          "Valve added five-item Covert contracts in its 22 October 2025 update. Five regular Covert inputs can produce a regular knife or gloves associated with their input collections; five StatTrak inputs produce StatTrak knives. This tool builds those pools from the input cases. It does not draw from unrelated cases. Inputs without a supported output route are excluded. Probabilities model equal shares within the available pool and do not distinguish rare pattern premiums." }
      ]
    },
  ...
  tr: {
    ...
    tradeup: {
      ...
      blocks: [
        ...
        { type: 'p', text:
          "Önce her girdinin float değeri kendi minimum ve maksimum aralığı içinde normalize edilir. Bu değerlerin ortalaması çıktı skininin aralığına aktarılır. Örneğin 0.10–0.70 aralığındaki 0.40 girdinin konumu 0.50 olur; 0.00–0.80 aralıklı çıktı 0.40 olur. Hesap sırasında hassasiyet korunur; fiyatlar tek tek float primine değil aşınma bantlarına bağlıdır." },
        ...
        { type: 'h3', text: 'Beşli Covert sözleşmeleri' },
        { type: 'p', text:
          "Valve, 22 Ekim 2025 güncellemesinde beşli Covert sözleşmelerini ekledi. Beş normal Covert girdiden ilgili koleksiyonların normal bıçak veya eldivenleri; beş StatTrak girdiden StatTrak bıçaklar çıkabilir. Bu araç havuzu girdilerin kasalarından oluşturur; ilgisiz kasalardan sonuç üretmez. Desteklenen çıktı yolu bulunmayan girdiler listelenmez. Olasılıklar mevcut havuz içindeki eşit payları modeller; nadir desen primleri ayrıca hesaplanmaz." }
      ]
    },
...
// Güncel fiyat metodolojisi iki dilde aynı içerik modeline eklenir.
GUIDE.en.tradeup.blocks.push({ type: 'h3', text: 'Price sources and limits' }, { type: 'p', text: "Price data uses Skinport suggested USD values where available and a Steam snapshot for coverage. The source date matters more than the download date. Old Steam data is not used to recalibrate fresh Skinport prices. Missing, old and modelled values are marked in the contract summary; an approximate value is not an executable quote. Edit input and outcome prices to compare your own market. Profit excludes selling fees, sticker premiums and individual float or pattern premiums." });
GUIDE.tr.tradeup.blocks.push({ type: 'h3', text: 'Fiyat kaynakları ve sınırlar' }, { type: 'p', text: "Fiyat verisinde mevcut olduğunda Skinport önerilen USD değerleri, kapsam yedeğinde Steam anlık görüntüsü kullanılır. Kaynağın tarihi indirme tarihinden önemlidir. Eski Steam verisi güncel Skinport fiyatlarını yeniden ölçeklemek için kullanılmaz. Eksik, eski ve modellenmiş değerler sözleşme özetinde işaretlenir; tahmini fiyat uygulanabilir satış teklifi değildir. Kendi piyasanız için girdi ve çıktı fiyatlarını düzenleyin. Kâr hesabı satış komisyonu, çıkartma primi, tekil float veya desen primlerini içermez." });

// Gizlilik açıklamaları etkin iletişim, ölçüm ve depolama davranışıyla uyumludur.
for (const lang of ['en', 'tr']) {
  const tr = lang === 'tr';
  const privacy = GUIDE[lang].privacy.blocks;
  privacy[2] = { type: 'p', text: tr ? 'İletişim formunu gönderirseniz adınız, e-posta adresiniz ve mesajınız FormSubmit aracılığıyla site sahibine iletilir. Steam hesabı veya ödeme bilgisi istemiyoruz. Form dışında oyun ilerlemesi tarayıcı oturumunda tutulur.' : 'If you submit the contact form, your name, email address and message are sent through FormSubmit to the site owner. We do not request Steam credentials or payment details. Simulation progress is held in your browser session.' };
  privacy[4] = { type: 'p', text: tr ? 'Tarayıcınızda tema tercihi ve sorumluluk reddi bildirimini kapatma tercihi saklanır. Bakiye, envanter ve geçmiş sayfa yenilenince silinir. Google Analytics ölçüm etiketi ve AdSense reklam etiketi de uygulama sayfasında bulunur; bu hizmetler kendi çerezlerini veya benzer teknolojileri kullanabilir.' : 'Your browser stores your theme preference and dismissal of the disclaimer. Balance, inventory and history are cleared on refresh. The app page also includes Google Analytics measurement and Google AdSense tags; these services may use their own cookies or similar technologies.' };
  privacy.push({ type: 'h3', text: tr ? 'Ölçüm ve iletişim sağlayıcıları' : 'Measurement and contact providers' }, { type: 'p', text: tr ? 'Google Analytics site kullanımını ölçmek için sayfa ziyaretleri ve cihaz/tarayıcı bilgilerini işleyebilir. FormSubmit iletişim mesajlarını taşır. GitHub ve Steam içerik sunucuları ağ istekleri sırasında IP adresi gibi bağlantı bilgilerini alır. Hizmetlerin saklama süreleri ve uygulamaları kendi gizlilik politikalarına bağlıdır. Gizlilik sorularınız için eolyrics@gmail.com adresine yazabilirsiniz.' : 'Google Analytics may process page visits and device/browser information to measure site usage. FormSubmit handles contact messages. GitHub and Steam content servers receive connection information such as IP addresses when resources are requested. Provider retention and processing follow their own privacy policies. Send privacy questions to eolyrics@gmail.com.' });
  GUIDE[lang].contact.blocks[2] = { type: 'p', text: tr ? 'Aşağıdaki e-posta adresinden bize ulaşabilirsiniz. Simülatörün sağ alt köşesindeki iletişim formunu da kullanabilirsiniz; gönderim başarısız olursa e-posta seçeneği sunulur.' : 'Use the email address below to contact us. You can also use the contact form at the bottom right of the simulator; it offers an email fallback if delivery fails.' };
  GUIDE[lang].contact.blocks[3].items = GUIDE[lang].contact.blocks[3].items.filter(item => !item.startsWith('Response time:') && !item.startsWith('Yanıt süresi:'));
}
