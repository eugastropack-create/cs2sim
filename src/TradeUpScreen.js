// 12 Eylül 2026: Etkin Covert kuralı tradeupRules.mjs içindedir; aşağıdaki eski
// "özel tarif" yorumları tarihsel kayıttır. Valve 22 Ekim 2025'te 5 Covert takasını ekledi.
import PriceStatus from './components/PriceStatus';
import { buildTradeupRoutes, calculateTradeup, defaultInputFloat } from './tradeupRules.mjs';
import { getPriceQuality } from './priceSnapshot.mjs';
import { buildMarketHashName } from './prices';
...
import { fetchSkins, fetchCrates } from './api';
...
function CompactFloatSlider({ value, min, max, onChange }) {
  ...
  const handleTextChange = (t) => {
    ...
    const n = /^(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)$/.test(cleaned) ? Number(cleaned) : NaN;
...
function TradeCard({ entry, index, cardWidth, locked, onPress, onLockedPress, onRemove, onClone, onFloatChange, onPriceChange, t }) {
  ...
  return (
    ...
    <View dataSet={{ tradecard: '1' }} style={[card.filled, { width: cardWidth, borderTopColor: entry.skin.rarity?.color || C.borderStrong }]}>
      ...
      <Text style={card.name} numberOfLines={1}>{entry.skin.isStatTrak ? 'StatTrak™ ' : ''}{entry.skin.name}</Text>
...
function SummaryContent({ analysis, filledCount, requiredCount, ready, profitAmount, profitPct, onOutcomePrice, t }) {
  const [visibleOutcomes, setVisibleOutcomes] = useState(OUTCOME_RENDER_CAP);
  useEffect(() => { if (!ready) setVisibleOutcomes(OUTCOME_RENDER_CAP); }, [ready]);
  if (!analysis) {
    return (
  ...
  return (
    <>
      <Text style={ts.sourceTxt}>{t(analysis.uncertain ? 'tradeup.uncertain' : 'tradeup.priceNote')}</Text>
      ...
      {analysis.outcomes.length === 0 ? (
        ...
        <>
          ...
          <View style={ts.outcomeList}>
                ...
                <View key={i} style={ts.outcomeRow}>
                  ...
                  <View style={ts.outcomeInfo}>
                    <Text style={[ts.outcomeName, { color: o.skin.rarity?.color || C.text }]} numberOfLines={1}>
                      {o.skin.isStatTrak ? 'StatTrak™ ' : ''}{o.skin.name}
                    </Text>
                  ...
                  <EditablePrice
                    prefix={!o.userPriced && o.quality !== 'market' ? '~' : ''}
                    value={o.price ?? 0}
            ...
            {analysis.outcomes.length > visibleOutcomes && (
              <TouchableOpacity onPress={() => setVisibleOutcomes(count => count + OUTCOME_RENDER_CAP)}><Text style={ts.outcomeMore}>{t('tradeup.showMore', { n: analysis.outcomes.length - visibleOutcomes })}</Text></TouchableOpacity>
            )}
          </View>
        </>
      )}
...
export default function TradeUpScreen({ inventory, setInventory, priceMap, allCollections, history = [], setHistory, pendingItem, onPendingItemHandled }) {
  ...
  const [allCrates, setAllCrates] = useState([]);
  ...
  useEffect(() => {
    ...
    Promise.all([fetchSkins(), fetchCrates()])
      .then(([data, crates]) => { if (!cancelled) { setAllSkins(data || []); setAllCrates(crates || []); setLoading(false); } })
  ...
  useEffect(() => {
    setSlots(previous => previous.map(entry => entry ? { ...entry, price: getRealisticPrice(priceMap, entry.skin, entry.float, !!entry.skin.isStatTrak, entry.skin.rarity?.name, false, { stable: true }) } : entry));
  }, [priceMap]);
  const routes = useMemo(() => buildTradeupRoutes(allSkins, allCollections || [], allCrates, stattrak), [allSkins, allCollections, allCrates, stattrak]);
  // BUG DÜZELTMESİ: Eski genel bıçak havuzu ve ilk 10 popüler eşya yedeği kaldırıldı.
  // Geçersiz/eksik veri hiçbir zaman geçerli bir sözleşme gibi hesaplanmaz.
  useEffect(() => {
    const model = calculateTradeup(slots, routes);
    if (!model) { setAnalysis(null); return; }
    const totalCost = slots.filter(Boolean).reduce((sum, entry) => sum + effectivePrice(entry), 0);
    const outcomes = model.outcomes.map(outcome => {
      const key = outcome.skin.id;
      const userPriced = outcomePrices[key] != null;
      const outWear = outcome.outFloat == null ? null : getWearFromFloat(outcome.outFloat);
      const quality = getPriceQuality(priceMap, buildMarketHashName(outcome.skin, outWear, stattrak));
      return { ...outcome, outWear, userPriced, quality,
        price: userPriced ? outcomePrices[key] : getRealisticPrice(priceMap, outcome.skin, outcome.outFloat, stattrak, outcome.skin.rarity?.name, false, { stable: true }) };
    });
    const uncertain = outcomes.some(outcome => !outcome.userPriced && outcome.quality !== 'market') || slots.filter(Boolean).some(entry => entry.userPrice == null && getPriceQuality(priceMap, buildMarketHashName(entry.skin, getWearFromFloat(entry.float), stattrak)) !== 'market');
    const ev = outcomes.reduce((sum, outcome) => sum + outcome.price * outcome.chance / 100, 0);
    const profitChance = outcomes.reduce((sum, outcome) => sum + (outcome.price > totalCost ? outcome.chance : 0), 0);
    setAnalysis({ ...model, outcomes, totalCost, ev, profitChance, uncertain });
  }, [slots, routes, priceMap, outcomePrices, stattrak]);
  ...
  useEffect(() => {
    ...
    const pendingSkin = { ...allSkins.find(skin => skin.id === pendingItem.id), ...pendingItem };
    const pendingRoutes = buildTradeupRoutes(allSkins, allCollections || [], allCrates, !!pendingItem.isStatTrak);
    if (!isValidTradeUpInput(pendingItem) || pendingItem.isSouvenir || !pendingRoutes.has(pendingItem.id) || (slots.some(Boolean) && !!pendingItem.isStatTrak !== stattrak)) {
    ...
    setStattrak(!!pendingItem.isStatTrak);
    ...
    setSlots(prev => {
      ...
      placed = true;
      const n = [...prev];
      n[free] = { skin: pendingSkin, float: pendingItem.float, price: getRealisticPrice(priceMap, pendingSkin, pendingItem.float, !!pendingItem.isStatTrak, pendingSkin.rarity?.name, false, { stable: true }) };
      for (let i = limit; i < n.length; i++) n[i] = null;
      return n;
    });
    setSubTab('contract');
    setTimeout(() => showToast(placed ? t('tradeup.added', { name: pendingItem.name }) : t('tradeup.full'), placed ? 'success' : 'error'), 0);
    done();
  }, [pendingItem, loading]);
  ...
  const handleSelect = (skin) => {
    if (!routes.has(skin.id)) { showToast(t('tradeup.invalidInput'), 'error'); return; }
    skin = { ...skin, isStatTrak: stattrak };
    const def = defaultInputFloat(skin);
    ...
    const price = getRealisticPrice(priceMap, skin, def, stattrak, skin.rarity?.name, false, { stable: true });
  ...
  const handleFloatChange = (idx, val) => {
    setSlots(prev => {
      ...
      const newPrice = getRealisticPrice(priceMap, n[idx].skin, val, !!n[idx].skin.isStatTrak, n[idx].skin.rarity?.name, false, { stable: true });
  ...
  const executeTradeUp = () => {
    ...
    setWonItem({ ...winner.skin, float: winner.outFloat, price: winner.price, displayColor: winColor, isKnifeWin: wonIsKnife, wear: winner.outWear, pattern: generatePattern(), acquiredAt: Date.now(), uid: Date.now().toString(), source: wonIsKnife ? 'TRADE-UP ★' : 'TRADE-UP' });
  ...
  const loadHistoryEntry = (entry) => {
    ...
    setStattrak(!!entry.slots[0]?.skin?.isStatTrak);
    setSlots(restored);
    setSubTab('contract');
  };
  ...
  if (loading) return <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}><ActivityIndicator color={C.accent} /></View>;
  ...
  const pickerPool = allSkins.filter(s => routes.has(s.id) && (!lockedRarity || s.rarity?.name === lockedRarity));
  ...
  const gridElement = (
    <View style={ts.gridPanel}>
      <PriceStatus priceMap={priceMap} />
      <View style={ts.gridPanelHeader}>
        <TouchableOpacity disabled={filledCount > 0} accessibilityRole="switch" accessibilityState={{ checked: stattrak, disabled: filledCount > 0 }} onPress={() => setStattrak(value => !value)}><Text style={{ color: C.activeTxt, fontSize: 11 }}>{stattrak ? 'StatTrak™' : t('tradeup.regular')}</Text></TouchableOpacity>
      ...
      {wonItem && (
        <View style={ts.resultOverlay}>
          <View style={[ts.resultBox, { borderColor: wonItem.displayColor }]}>
            ...
            <Text style={{color: wonItem.displayColor, fontSize: 18, fontWeight: 'bold', textAlign: 'center'}}>{wonItem.isStatTrak ? 'StatTrak™ ' : ''}{wonItem.name}</Text>
            {Number.isFinite(wonItem.float) && <Text style={{color: C.text, fontSize: 14, marginVertical: 8}}>{t('tradeup.avgFloat')}: {wonItem.float.toFixed(4)} ({wonItem.wear})</Text>}
