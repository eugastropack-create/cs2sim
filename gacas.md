...
| **2026-09-02** | **KRİTİK — TRADE-UP ÇIKTI FLOAT'I UYDURMA ARALIKLA HESAPLANIYORDU.** Girdiler `skins.json`'dan (aralıklar dolu) ama ÇIKTILAR `collections.json`'dan geliyordu ve o dosyanın 1455 skininin **hiçbirinde** `min_float`/`max_float` yok. `?? 0` / `?? 1` yedeği yüzünden çıktıların **%75.9'u** 0.00–1.00 varsayımıyla hesaplanıyordu. İki semptom birden üretiyordu: (1) imkânsız aşınma — `SSG 08 \| Slashed` 0.15'ten başlar, FN olamaz; (2) yanlış fiyat — sonra var olmayan bir `market_hash_name` kurulup canlı fiyat kaçırılıyordu. Ölçülen düzelme: 10× SSG 08 \| Slashed sözleşmesinde `MAC-10 \| Curse` 0.0769 **MW** → 0.0385 **FN**, `XM1014 \| Heaven Guard` 0.0769 **MW** → 0.0662 **FN**. Aralık artık `skins.json`'dan KİMLİKLE getiriliyor |
| **2026-09-02** | **Fiyat sağlayıcı adaptörü** (`scripts/price-providers.mjs`): Skinport (ücretsiz, anahtarsız, VARSAYILAN) · cs2.sh (`CS2SH_API_KEY`) · CSFloat (`CSFLOAT_API_KEY`). Hepsi aynı sözleşmeyi uygular; sağlayıcı `PRICE_PROVIDERS` ile tek satırda değişir. Anahtar yoksa/biterse sağlayıcı kendini atlar → site ÇÖKMEZ (anahtarsız koşu doğrulandı) |
| **2026-09-02** | **Ölçüm: cs2.sh 1. madde için gereksiz.** `/v1/schema` içindeki `float_range`, `skins.json` ile 2.106 skinde **birebir aynı** (tek fark yok). Ücretli ve 2 günlük bir bağımlılık, zaten ücretsiz ve kalıcı olan veri için kurulmadı |
| **2026-09-02** | **Ölçüm: CSFloat "ücretsiz public API" değil.** `api/v1/listings` üç farklı User-Agent ile de **403** ("You need to be logged in to search listings"). Gerçek ücretsiz yedek Skinport'tur |
| **2026-09-02** | **KRİTİK — FİYATLAR 24.6 GÜNDÜR BAYATTI.** ByMykel/Steam beslemesi 2026-08-08'de donmuş; toplayıcı Steam'in hız sınırları yüzünden turu hiç tamamlayamamış. Cron'umuz 2 saatte bir çalışsa da AYNI bayat sayıları yayınlıyordu — **cron sıklığı, kaynak donmuşken hiçbir şey çözmez.** Roller tersine çevrildi: **Skinport birincil fiyat** (canlı, ölçümde 2 dk önce güncellenmişti), Steam yalnızca kapsam yedeği. Alan da düzeltildi: `median_price` stickerlı/nadir desenli ilanlarla şişiyor (Redline FT median $50.47 / suggested $36.68), doğrusu **`suggested_price`**. Skinport üçüncü taraf olduğu için Steam seviyesine kalibre ediliyor (10.560 likit eşyada ölçülen medyan oran **×1.256**, sabit yazılmıyor). Sonuç: `M4A4 \| Howl (FT)` $4.853 → **$6.392**, `AWP \| Dragon Lore (FN)` $13.168 → **$17.267**. Kırık aşınma eğrisi **%12 → %4.9** |
| **2026-09-02** | **Sessiz bayatlamaya karşı tazelik denetimi:** her kaynağın yaşı ölçülüp `metadata`ya yazılıyor, 3 günden eskiyse Actions kaydına UYARI basılıyor. Önceki sefer hiçbir uyarı olmadığı için sorun haftalarca fark edilmedi |
| **2026-09-02** | **Fiyat alanı belirginleştirildi** — kullanıcı "düzenlenebilir olduğu anlaşılmıyor" dedi. Dört ipucu birden: kalem ikonu, kesikli alt çizgi, hover'da zemin/çerçeve/ikon rengi değişimi, `cursor: text`. Native'de hover olmadığı için ipucu orada daima görünür |
| **2026-09-01** | **Düzenlenebilir fiyat (TradeUpLab mantığı).** Girdi kartındaki ve çıktı satırındaki fiyata tıklanınca yerinde bir `<input>` açılıyor; kullanıcı eşyayı üçüncü taraf bir siteden aldığı gerçek fiyatı girebiliyor. Toplam maliyet, Beklenen Değer, kâr ve **kâr ihtimali** anında yeniden hesaplanıyor. ⚠️ ÇEKİLİŞ İHTİMALLERİ DEĞİŞMİYOR — aksi hâlde kullanıcı fiyat yazarak kendi kazanma şansını değiştirebilirdi. Doğrulandı: bir girdi $0.28→$1.50 yapılınca maliyet $2.80→$4.02 ve kâr +$0.05→−$1.17; çıktı $4.87→$40.00 yapılınca EV $2.85→$20.82 (=0.5×40+0.5×1.64) ve kâr ihtimali %0→%50; oranlar %50/%50'de sabit kaldı |
| **2026-09-01** | **Fiyat boru hattı YAYINA GİRDİ.** `prices-data` dalı canlı: 35.156 eşya, 4.532 fiyat onarıldı. Kullanıcının bildirdiği `USP-S \| Bleeding Edge (Well-Worn)` $3.91 → **$1.02**; `AWP \| Dragon Lore (Factory New)` simüle $103 yerine gerçek **$17.849,70** |
| **2026-09-01** | **Global arama eşyaları da buluyor.** Önceden arama yalnızca KUTU döndürüyordu: "AWP | Dragon Lore" yazan kullanıcı onu içeren kasaları görüyor, skinin kendisini/fiyatını göremiyordu. Artık eşyalar ayrı bir bölümde listeleniyor ve tıklanınca aşınmaya göre fiyat + hangi kutulardan çıktığı açılıyor. Sıralama düzeltildi: ham konum sıralaması "Dragon Lore (Foil)" sticker'ını başa koyuyordu; artık `\|` sonrası eşleşme de baştan sayılıyor ve silah skinleri öne alınıyor |
| **2026-09-01** | **`skins.json` App'e de yüklendi (önbellekli, ek indirme YOK).** Sebep: `min_float`/`max_float` alanları `crates.json` ve `collections.json` kayıtlarının hiçbirinde yok (0/17.583 ve 0/1.455 — ölçüldü). Bu yüzden arama kartı önce "Float aralığı 0.00–1.00" gibi yanlış bir bilgi gösteriyordu |
| **2026-09-01** | **Simüle fiyat artık dürüstçe işaretleniyor** (`hasLivePrice`): canlı kayıt yoksa değer `~` önekiyle soluk basılıyor ve uyarı çıkıyor. AWP | Dragon Lore ham ByMykel tablosunda hiç yok; simülasyon $103 gösteriyordu, gerçeği ~$17.800 |
| **2026-09-01** | **Trade-Up'ta arama kutusu ve reklam boşluğu gizlendi** — ölçülen kazanç 169 px; 10 yuvalık ızgara artık kaydırmadan görünüyor |
| **2026-09-01** | **Logoya tıklama ana menüye döndürüyor** — mini logo da dahil (eskiden yalnızca listeyi başa sarıyordu, sekme değişmiyordu) |
| **2026-09-01** | **Yeni ejderha logosu siteye alındı.** Kaynak JPEG'in beyaz zemini ölçülen eşiklerle saydamlaştırıldı: arka plan min-kanal ≥252 / doygunluk ≤2, en açık krem harf min-kanal 214 / doygunluk 36 — aradaki geniş boşluk sayesinde harflere hiç dokunulmadı. Kenarlarda beyaz geri çıkarıldı (un-premultiply), böylece koyu zeminde hale kalmadı. Oran **3.6885** (eski wordmark 9.82); bu yüzden logo ölçüsü artık genişlikten değil **yükseklikten** türetiliyor — aksi hâlde başlık 74 px şişiyordu. Favicon ejderha kafasından kırpıldı |
...
## 12 Eylül 2026 — Fiyat ve trade-up doğruluk düzeltmesi

Bu bölüm önceki tarihsel kuralların değişen kısımlarının yerini alır.
- Etkin hesaplama: src/tradeupRules.mjs. Normal sözleşme 10, Covert 5 girdi. Valve 22 Ekim 2025 güncellemesi Covert takasını destekler. Eski “özel tarif” ve tüm bıçaklara eşit erişim açıklamaları artık geçerli değildir.
- Havuz, girdi koleksiyon/kasa kimliklerinden oluşturulur. Koleksiyonu olmayan girdiye popüler eşya yedeği verilmez. StatTrak ayrı; Souvenir ve varyant karışımı reddedilir. Float aralığı bilinmeyen kayıt hesaplanmaz. Ara float yuvarlaması yoktur.
- Kaynak güncelliği src/priceSnapshot.mjs içinde denetlenir. JSON details alanı her fiyatın source, updated_at, estimated bilgisini taşır. Eski şema “doğrulanmamış” sayılır.
- 35 günlük Steam verisiyle Skinport kalibrasyonu yapılmaz; yalnızca taze Steam ile ölçülen kalibrasyon uygulanır. Kalibre edilmiş fiyat estimated etiketi alır. Monoton aşınma zorunluluğu piyasa kuralı değildir; tarihsel repairWearCurves ancak REPAIR_WEAR_CURVES=true ile seçilebilir ve tahmin olarak işaretlenir.
- Normal, StatTrak ve Souvenir fiyatları birbirinin yerine kullanılmaz. Eksik fiyat simülasyondur; kâr paneli bunu açıklar. Negatif/harfli fiyat girdileri reddedilir.
- Ağ istekleri api.js içinde, süre sınırlı ve JSON doğrulamalıdır. Skinport sağlayıcısı belgelenen Brotli başlığını gönderir. API anahtarları yalnızca mevcut GitHub Secrets alanında kalır.
- TradeUpLab için belgelenmiş halka açık API bulunmadı; özel uç noktaya bağımlılık eklenmedi. Kaynak: https://docs.skinport.com/items ve Valve: https://store.steampowered.com/news/app/730/view/516350567072662004
- npm test hesaplama/veri regresyonlarını; npm run build:web taranabilir rehberleri ve Expo web çıktısını üretir. Eski hesaplama hata yorumları TRADEUP-LEGACY-NOTES.md içinde korunur.

- Web rehberleri public/guides/en ve public/guides/tr altında 12 bağımsız HTML sayfasıdır; kaynak metin src/content/guide.js. npm run build:web bunları yeniden üretir. Sitemap ve robots dosyaları dahil edilir. Uygulama içi rehber tıklamaları oturumu korur; href adresleri doğrudan erişim sağlar.
- public/prices/bootstrap.json, ilk yayın için 12 Eylül tarihli bir başlangıç anlık görüntüsüdür. Eski şemalı veya erişilemeyen prices-data beslemesinden sonra denenir. Kalıcı güncellik için GitHub Actions çalışmalıdır; bu dosya canlı API değildir. Eski fiyatlar yeniden tarihlenmez.
- StatTrak bıçak market adı: yıldız önce gelir (★ StatTrak™ Bayonet). Vanilla bıçakta bilinmeyen aşınma gösterilmez. Salt karşılaştırma fiyatlarında rastgele mock varyansı kapatıldı.
- İletişim sayfasındaki yayın öncesi taslak kaldırıldı; FormSubmit, Analytics, AdSense ve tema depolaması gizlilik metninde açıklanır. AdSense/Analytics kimlikleri korundu. Sertifikalı CMP yayın ayarı koddan doğrulanamaz; hesap sahibi AdSense Privacy & messaging bölümünde kontrol etmelidir.
