...
export const skinportProvider = {
  ...
  async fetchPrices() {
    ...
    for (const it of data) {
      ...
      const price = it.suggested_price;
      if (!Number.isFinite(price) || price <= 0) continue;
      out.set(it.market_hash_name, { price, qty: it.quantity ?? 0, updatedAt: it.updated_at ? new Date(it.updated_at * 1000).toISOString() : null });
