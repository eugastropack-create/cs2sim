...
import { isStaleDate } from '../src/priceSnapshot.mjs';
...
async function getJson(url, label) {
  const res = await fetch(url, { signal: AbortSignal.timeout(30000), headers: { 'User-Agent': 'skinsimulator-price-bot' } });
...
function measureCalibration({ skinport, steam, steamUpdatedAt }) {
  // KÖK NEDEN: 35 günlük Steam anlık görüntüsü güncel Skinport'u bozamaz.
  if (isStaleDate(steamUpdatedAt)) { log('⚠️ Eski Steam verisiyle kalibrasyon atlandı; kaynak USD fiyatı korunuyor'); return 1; }
  ...
  for (const key of keys) {
    ...
    if (sp != null) {
      ...
      merged.set(key, {
        ...
        src: sources.providerId || 'skinport', updated_at: sp.updatedAt || sources.skinportUpdatedAt, estimated: calibration !== 1
      });
    } else if (st != null) {
      ...
      fromSteam++;
      merged.set(key, { price: st, qty: hasWear ? 0 : MIN_QUANTITY, trusted: !hasWear, src: 'steam', updated_at: sources.steamUpdatedAt, estimated: false });
    }
  }
  ...
  for (const [base, row] of groups) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (Math.abs(price - row[i].price) / row[i].price > 0.05) {
        merged.set(row[i].key, { ...row[i], price, trusted: false, src: 'repaired', estimated: true });
        repaired++;
      }
    }
  }
...
async function main() {
  ...
  // Fiyatların aşınmayla monoton azalması garanti değildir; gerçek veriyi varsayımla değiştirmeyiz.
  // Tarihsel onarım modeli yalnızca açıkça seçilirse uygulanır ve tahmin olarak etiketlenir.
  const mergedRaw = mergeSources(sources);
  const merged = process.env.REPAIR_WEAR_CURVES === 'true' ? repairWearCurves(mergedRaw) : mergedRaw;

  const prices = {};
  const details = {};
  for (const [k, v] of merged) if (v.price > 0) prices[k] = parseFloat(v.price.toFixed(2));

  for (const [key, value] of merged) details[key] = { source: value.src, updated_at: value.updated_at || null, estimated: !!value.estimated };
  const srcList = [];
  if (sources.skinport) srcList.push(sources.providerLabel || sources.providerId);
