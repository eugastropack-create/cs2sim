...
| **2026-09-02** | **Fiyat sağlayıcı adaptörü** — `scripts/price-providers.mjs`. Sağlayıcılar: Skinport (ücretsiz, anahtarsız, VARSAYILAN), cs2.sh (`CS2SH_API_KEY` Secret'ı), CSFloat (`CSFLOAT_API_KEY` Secret'ı). ⚠️ Anahtarlar KODA YAZILMAZ, Actions Secret'ından `env` ile geçer. Anahtar yoksa sağlayıcı kendini atlar ve boru hattı ücretsiz kaynağa düşer — iş başarısız olmaz |
| **2026-09-02** | ⚠️ **CSFloat ücretsiz DEĞİL:** `api/v1/listings` girişsiz 403 döndürüyor (ölçüldü). "Ücretsiz yedek plan" olarak Skinport kullanılıyor |
